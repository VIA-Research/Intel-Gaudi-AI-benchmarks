...
bool register_custom_scale_bf16_unroll2() {
    // Registering custom_op::custom_scale_bf16_unroll2
    // inputs desc
    ...
    REGISTER_CUSTOM_OP_ATTRIBUTES(
        "custom_op::custom_scale_bf16_unroll2", //schema name
        "custom_scale_bf16_unroll2_gaudi2", // guid
        inputs_desc,
        outputs_desc,
        user_params_lambda);
    std::cout << "cpp registered custom_op::custom_scale_bf16_unroll2\n";
    return true;
}

at::Tensor custom_scale_bf16_unroll2_execute(
    torch::Tensor inputA,
    c10::Scalar scalar) {
  ...
  static bool registered = register_custom_scale_bf16_unroll2();
  TORCH_CHECK(registered, "custom_scale_bf16_unroll2 kernel not registered" );
  std::vector<c10::IValue> inputs{inputA, scalar};
  // Get custom op descriptor from registry
  auto op_desc = habana::custom_op::HabanaCustomOpDescriptor::getCustomOpDescriptor("custom_op::custom_scale_bf16_unroll2");
...
TORCH_LIBRARY(custom_op, m) {
  m.def("custom_scale_bf16_unroll2(Tensor self, Scalar side) -> Tensor");
}
TORCH_LIBRARY_IMPL(custom_op, HPU, m) {
  m.impl("custom_scale_bf16_unroll2", custom_scale_bf16_unroll2_execute);
}
