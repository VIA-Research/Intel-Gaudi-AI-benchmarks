...
bool register_custom_triad_bf16_unroll1() {
    // Registering custom_op::custom_triad_bf16_unroll1
    // inputs desc
    ...
    REGISTER_CUSTOM_OP_ATTRIBUTES(
        "custom_op::custom_triad_bf16_unroll1", //schema name
        "custom_triad_bf16_unroll1_gaudi2", // guid
        inputs_desc,
        outputs_desc,
        user_params_lambda);
    std::cout << "cpp registered custom_op::custom_triad_bf16_unroll1\n";
    return true;
}

at::Tensor custom_triad_bf16_unroll1_execute(
    torch::Tensor inputA,
    torch::Tensor inputB,
    c10::Scalar scalar) {
  ...
  static bool registered = register_custom_triad_bf16_unroll1();
  TORCH_CHECK(registered, "custom_triad_bf16_unroll1 kernel not registered" );
  std::vector<c10::IValue> inputs{inputA, inputB, scalar};
  // Get custom op descriptor from registry
  auto op_desc = habana::custom_op::HabanaCustomOpDescriptor::getCustomOpDescriptor("custom_op::custom_triad_bf16_unroll1");
...
TORCH_LIBRARY(custom_op, m) {
  m.def("custom_triad_bf16_unroll1(Tensor self, Tensor other, Scalar side) -> Tensor");
}
TORCH_LIBRARY_IMPL(custom_op, HPU, m) {
  m.impl("custom_triad_bf16_unroll1", custom_triad_bf16_unroll1_execute);
}
