...
bool register_custom_add_bf16_unroll1() {
    // Registering custom_op::custom_add_bf16_unroll1
    // inputs desc
    ...
    REGISTER_CUSTOM_OP_ATTRIBUTES(
        "custom_op::custom_add_bf16_unroll1", //schema name
        "custom_add_bf16_unroll1_gaudi2", // guid
        inputs_desc,
        outputs_desc,
        nullptr);


    std::cout << "cpp registered custom_op::custom_add_bf16_unroll1\n";
    return true;
}


at::Tensor custom_add_bf16_unroll1_execute(
    torch::Tensor inputA,
    torch::Tensor inputB) {
  ...
  static bool registered = register_custom_add_bf16_unroll1();
  TORCH_CHECK(registered, "custom_add_bf16_unroll1 kernel not registered" );
  std::vector<c10::IValue> inputs{inputA, inputB};
  // Get custom op descriptor from registry
  auto op_desc = habana::custom_op::HabanaCustomOpDescriptor::getCustomOpDescriptor("custom_op::custom_add_bf16_unroll1");
...
TORCH_LIBRARY(custom_op, m) {
  m.def("custom_add_bf16_unroll1(Tensor self, Tensor other) -> Tensor");
}
TORCH_LIBRARY_IMPL(custom_op, HPU, m) {
  m.impl("custom_add_bf16_unroll1", custom_add_bf16_unroll1_execute);
}
