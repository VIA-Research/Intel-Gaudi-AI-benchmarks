...
echo "Generated values for M, K, N: ${M_values[@]}, ${fixed_size}"

# Run the loop for M, K, N values
for value in "${M_values[@]}"
do
	output_file="./trace_files/${value}_${value}_${fixed_size}_${dtype}.csv"
	if [ -f "$output_file" ]; then
        echo "File $output_file already exists. Skipping..."
        continue
    fi

    echo $value, $value, $fixed_size
    python gemm.py --m=$value --k=$value --n=$fixed_size --dtype=$dtype
    mv *_analyzed_nodes.csv ${value}_${value}_${fixed_size}_${dtype}.csv
	mv ${value}_${value}_${fixed_size}_${dtype}.csv ./trace_files
    rm default_*
done
