...
bool register_custom_triad_Nop_bf16() {
    // Registering custom_op::custom_triad_Nop_bf16
    // inputs desc
    ...
    auto user_params_lambda = [](const at::Stack& inputs, size_t& size) {
      ...
      params->N_op = inputs[3].toInt(); // bottom
      return params;
    };

    // actual register
    REGISTER_CUSTOM_OP_ATTRIBUTES(
        "custom_op::custom_triad_Nop_bf16", //schema name
        "custom_triad_Nop_bf16_gaudi2", // guid
        inputs_desc,
        outputs_desc,
        user_params_lambda);
    std::cout << "cpp registered custom_op::custom_triad_Nop_bf16\n";
    return true;
}

at::Tensor custom_triad_Nop_bf16_execute(
    torch::Tensor inputA,
    torch::Tensor inputB,
    c10::Scalar scalar,
    c10::Scalar Nop) {
  ...
  static bool registered = register_custom_triad_Nop_bf16();
  TORCH_CHECK(registered, "custom_triad_Nop_bf16 kernel not registered" );
  std::vector<c10::IValue> inputs{inputA, inputB, scalar, Nop};
  // Get custom op descriptor from registry
  auto op_desc = habana::custom_op::HabanaCustomOpDescriptor::getCustomOpDescriptor("custom_op::custom_triad_Nop_bf16");
...
TORCH_LIBRARY(custom_op, m) {
  m.def("custom_triad_Nop_bf16(Tensor self, Tensor other, Scalar X, Scalar Y) -> Tensor");
}
TORCH_LIBRARY_IMPL(custom_op, HPU, m) {
  m.impl("custom_triad_Nop_bf16", custom_triad_Nop_bf16_execute);
}
