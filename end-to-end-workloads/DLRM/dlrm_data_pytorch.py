...
class RandomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        elif self.data_generation == "random_gaudi":
            (X, lS_o, lS_i) = generate_dist_input_batch_gaudi(
                self.m_den,
                self.ln_emb,
                n,
                self.num_indices_per_lookup,
...
# random data from uniform or gaussian ditribution (input data)
def generate_dist_input_batch_gaudi(
    m_den,
    ln_emb,
    n,
    num_indices_per_lookup,
    ...
    for size in ln_emb:
        ...
        for _ in range(n):
            ...
            if rand_data_dist == "gaussian":
                ...
                sparse_group = np.random.permutation(size)[:sparse_group_size]
            else:
                raise (
                    rand_data_dist,
                    "distribution is not supported. \
